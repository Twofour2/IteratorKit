...
{
    public class SRSOracleGraphics : OracleGraphics
    {
       // public new SRSOracle oracle;
        public new SRSOracle oracle
        {
            get
            {
                return base.owner as SRSOracle;
            }
        }
        public bool IsSuns = true;

        public int sunSprite;

        public int sunFins;

        public SRSOracleGraphics(PhysicalObject ow, SRSOracle oracle) : base(ow)
        {
            //this.oracle = oracle;
            ...
            {
                this.armJointGraphics[i] = new SRSOracleGraphics.ArmJointGraphics(this, this.oracle.arm.joints[i], this.totalSprites);
            ...
            this.armBase = new SRSOracleGraphics.SRSArmBase(this, this.firstArmBaseSprite);
        ...
        public override void ApplyPalette(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, RoomPalette palette)
        {
            ...
            Color oracleColor = new Color(0.94f, 0.82f, 0.29f);
            for (int j = 0; j < base.owner.bodyChunks.Length; j++)
            {
                sLeaser.sprites[this.firstBodyChunkSprite + j].color = oracleColor;
            }
            ...
            for (int k = 0; k < 2; k++)
            {
                ...
                if (this.gown != null)
                {
                    Color gownColor = this.gown.Color(0.4f);
                    for (int l = 0; l < 7; l++)
                    {
            ...
            sLeaser.sprites[this.sunSprite].color = new Color(0.92f, 0.25f, 0.20f);

            sLeaser.sprites[this.sunFins].color = new Color(1f, 0f, 0f);
        ...
        public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
        {
           ...
           // base.InitiateSprites(sLeaser, rCam);
            ...
            sLeaser.sprites[this.sunSprite] = new FSprite("sunSigil", true); // sunSigil
            sLeaser.sprites[this.sunSprite].color = new Color(0.92f, 0.25f, 0.20f);

            sLeaser.sprites[this.sunFins] = new FSprite("sunFins", true);
        ...
        public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            ...
            sLeaser.sprites[this.sunSprite].x = sunVector.x - camPos.x;
            sLeaser.sprites[this.sunSprite].y = sunVector.y - camPos.y;
            sLeaser.sprites[this.sunSprite].rotation = Custom.AimFromOneVectorToAnother(sunVector, headVector - vector6 * 10f);
            sLeaser.sprites[this.sunSprite].scaleX = Mathf.Lerp(0.4f, 0.3f, Mathf.Abs(lookVector.x));
            sLeaser.sprites[this.sunSprite].scaleY = Custom.LerpMap(lookVector.y, 0f, 0.4f, 0.4f, 0.1f);


            sLeaser.sprites[this.sunFins].x = (sunVector.x - camPos.x);
            sLeaser.sprites[this.sunFins].y = (sunVector.y - camPos.y) + 0.7f;
            sLeaser.sprites[this.sunFins].rotation = Custom.AimFromOneVectorToAnother(sunVector, headVector - vector6 * 10f);
            sLeaser.sprites[this.sunFins].scaleX = Mathf.Lerp(0.7f, 0.5f, Mathf.Abs(lookVector.x));
            sLeaser.sprites[this.sunFins].scaleY = Custom.LerpMap(lookVector.y, 0.8f, 1f, 0.8f, 1f); // todo: make this look right
        ...
        {
            public new SRSOracleGraphics owner;
        ...
        public class SRSGown
        {
            public static Color SRSColor(On.OracleGraphics.Gown.orig_Color orig, OracleGraphics.Gown self, float f)
            {
                ...
                if (origRes == Custom.HSL2RGB(Mathf.Lerp(0.08f, 0.02f, Mathf.Pow(f, 2f)), Mathf.Lerp(0.6f, 0.4f, f), 0.4f))
                {
                    ...
                    // 79, 48, 104
                    // 273°, 37%, 30%
                    return Custom.HSL2RGB(Mathf.Lerp(0.78f, 0.76f, Mathf.Pow(f, 2f)), Mathf.Lerp(0.6f, 0.42f, f), 0.4f);
                }
                else
                {
                    return origRes;
