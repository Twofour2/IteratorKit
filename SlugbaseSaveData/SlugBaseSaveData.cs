    ...
    public class SlugBaseSaveData
    {
        ...
        private bool TryGetRaw<T>(string key, out T value)
        {
            ...
            if (LoadStringFromUnrecognizedStrings(key, out var stringValue))
            {
                ...
                {
                    IteratorKit.IteratorKit.Log.LogError($"Failed to convert key \"{key}\" to {typeof(T).Name}: \"{stringValue}\"");
        ...
        internal void SaveToStrings(List<string> strings)
        {
            foreach(var pair in _data)
            {
                ...
                {
                    IteratorKit.IteratorKit.Log.LogError($"Failed to serialize key \"{pair.Key}\" of type {pair.Value?.GetType().Name ?? "null"}!");
                    UnityEngine.Debug.LogException(e);
                }
            }
        }
        ...
        internal bool LoadStringFromUnrecognizedStrings(string key, out string value)
        {
            for(int i = 0; i < _unrecognizedSaveStrings.Count; i++)
            {
                ...
                if (s.StartsWith(key))
                {
                    ...
                    {
                        IteratorKit.IteratorKit.Log.LogError($"Badly formatted save data for key \"{key}\": \"{s.Substring(key.Length)}\"");
                        UnityEngine.Debug.LogException(e);
